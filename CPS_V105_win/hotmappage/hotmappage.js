(function () {
  ...
  /* ========== Electron IPC 数据存储（使用数据库预计算数据） ========== */
  var api = window.electronAPI;

  /* ========== 数据加载：从数据库获取预计算的每日秒数映射 ========== */
  async function loadAndBuildCache() {
    // 直接从数据库的 daily_focus 表获取 { dateStr: totalSeconds }
    var daySecondsMap = await api.getAllDayMap();

    // Step 1: 计算阈值
    var thresholds = calcThresholds(daySecondsMap);

    // Step 2: 预计算 dayLevels
    var dayLevels = {};
  ...
  (async function init() {
    // 监听数据库变更事件（替代 5 秒轮询）
    api.onDataChanged(async function () {
