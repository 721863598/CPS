(function () {
  ...
  function splitRecordByDay(record) {
    ...
    var sTime = record.startTime.substring(11, 16); // "HH:MM"
    ...
    // 按分钟比例分配秒数
    var assigned = 0;
    var dates = []; // [{dateStr, minutes}]

    // 开始日：从 sH:sM 到 24:00
    var startMin = 24 * 60 - (sH * 60 + sM);
    dates.push({ d: sDate, m: startMin });
    ...
    // 结束日：从 00:00 到 eH:eM
    var endMin = eH * 60 + eM;
    if (endMin > 0 || sDate === eDate) {
      dates.push({ d: eDate, m: endMin });
    }

    // 分配秒数，最后一个用余数确保总和精确
    for (var i = 0; i < dates.length; i++) {
      var frac = dates[i].m / totalMin;
      var sec;
      if (i === dates.length - 1) {
        sec = durSec - assigned; // 余数全部归最后一天
      } else {
        sec = Math.round(durSec * frac);
        assigned += sec;
      }
      if (sec > 0) {
  ...
  function processRecords(records) {
    ...
    for (var i = 0; i < records.length; i++) {
      var r = records[i];
      var splits = splitRecordByDay(r);
      for (var dateKey in splits) {
        if (splits.hasOwnProperty(dateKey)) {
          if (map[dateKey] === undefined) {
            map[dateKey] = splits[dateKey];
          } else {
            map[dateKey] += splits[dateKey];
          }
        }
