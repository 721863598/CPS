(function () {
  ...
  function filterByPeriod(records) {
    ...
    for (var i = 0; i < records.length; i++) {
      ...
      if (!timeStr) continue;
      var dateKey = timeStr.substring(0, 10); // "YYYY-MM-DD"
      if (inRange(dateKey, range.startStr, range.endStr)) {
        filtered.push(r);
      }
    }
    return filtered;
  ...
  function aggregateByTag(records) {
    ...
    for (var i = 0; i < records.length; i++) {
      ...
      map[tag].count += 1;
      map[tag].totalSeconds += parseDuration(r.duration);
    }
    return map;
  }
  ...
  function renderAll() {
    readRecordsFromFile().then(function (records) {
      ...
      for (var i = 0; i < filtered.length; i++) {
        totalSeconds += parseDuration(filtered[i].duration);
      }

      // 标签聚合
      var tagMap = aggregateByTag(filtered);
      ...
      if (records && records.length > 0) {
        ...
        for (var j = 0; j < records.length; j++) {
          var r = records[j];
          var timeStr = r.startTime || r.endTime;
          if (timeStr && inRange(timeStr.substring(0, 10), prevRange.startStr, prevRange.endStr)) {
            prevPeriodSeconds += parseDuration(r.duration);
          }
        }
      }
  ...
  function renderHistory(allRecords) {
    ...
    var htmlParts = [];
    for (var j = 0; j < yearRecords.length; j++) {
      var rec = yearRecords[j];
      var tag = rec.tag || '无标签';
      var startFormatted = formatDateTime(rec.startTime);
      var plainText, bodyHTML;

      if (rec.isComplete === '否' || !rec.endTime) {
        plainText = tag + startFormatted + '----未结束，进行中';
        bodyHTML = '<span class="hi-time">' + startFormatted + '</span>' +
                   '<span class="hi-arrow">----</span>' +
                   '<span class="hi-ongoing">未结束，进行中</span>';
      } else {
        var endFormatted = formatDateTime(rec.endTime);
        var durChinese = formatDurationChinese(rec.duration);
        plainText = tag + startFormatted + '----' + endFormatted + '专注' + durChinese;
        bodyHTML = '<span class="hi-time">' + startFormatted + '</span>' +
                   '<span class="hi-arrow">----</span>' +
                   '<span class="hi-time">' + endFormatted + '</span>' +
                   '<span class="hi-dur">专注' + durChinese + '</span>';
      }

      var fontSize = calcItemFontSize(plainText, containerWidth - 110); // 减去 tag badge 约 100px
      var titleText = tag + ' · ' + startFormatted;

      htmlParts.push(
        '<div class="history-item" style="font-size:' + fontSize.toFixed(1) + 'px" title="' + escapeHTML(titleText) + '">' +
        '<button class="hi-delete-btn" data-starttime="' + escapeHTML(rec.startTime || '') + '">×</button>' +
        '<span class="hi-tag">' + escapeHTML(tag) + '</span>' +
        bodyHTML +
        '</div>'
      );
    }
  ...
  historyList.addEventListener('click', async function (e) {
    var btn = e.target.closest('.hi-delete-btn');
    if (!btn) return;
    e.stopPropagation();
